...
def run_mode(mode):
    ...
    print("=" * 50)
    print(
        f"EXIT  : {EXIT_HOUR}:{EXIT_MINUTE:02d}"
    )
    print(
        f"BUY   : {BUY_HOUR}:{BUY_MINUTE:02d}"
    )
    print()
...
def run_auto():
    ...
    while True:
        ...
        if now.weekday() < 5:
            ...
            if (
                hour == BUY_HOUR
                and minute == BUY_MINUTE
                and not executed_buy_today
            ):
...
def main():
    ...
    buy_candidates = [
        ...
        if candidate["score"] >= MINIMUM_BUY_SCORE

    ]

    print(
        f"Candidates after score filter: "
